...
class PyLinXController(PyLinXData.PyLinXCoreDataObjects.PX_IdObject):
    ...
    def __init__(self, root = None, mainWindow = None, bListActions = True, name = u"controller"):
        ...
        self.__dictQtSignals = BContainer.BDict({}, name = u"dictQtSignals" )
        self.paste(self.__dictQtSignals)
    ...
    def get(self, attr):
        ...
        elif attr == u"Selection_types":
            return self.__get_Selection_types()
        elif attr[0] == u"@":
            attrSplit = attr.split(u".")
            dictAlias = self.getb("dictAlias")
            if attrSplit[0] in dictAlias:
                context = dictAlias[attrSplit[0]]
                return context.get(attr)          
        else:
            return super(PyLinXController, self).get(attr) 
    ...
    def set(self, attr, value, options = None):         
        if attr == u"Selection_listKeys":
            self.selection = value
        else:
            return super(PyLinXController, self).set(attr,value, options)    
